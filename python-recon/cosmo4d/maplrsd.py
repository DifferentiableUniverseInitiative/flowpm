## Main model in redshift space
## use NN to generate halo mass field and move them in redshift space
## with the reconstructed velocity field 
## optimize using loss function of log(M_R + M_0) form.
...
class MockModel(base.MockModel):
    def __init__(self, dynamic_model, ppath, mpath, zz, pwidth = None, Rrsd=3, mf=None):
    ...
    def _rsdfac(self):
        ...
        #rsdfac *= 3 ######################################
        if self.pm.comm.rank ==0: print('\nrsd factor for velocity in mock_model is = %0.2f\n'%rsdfac)
        return rsdfac
    ...
    def get_code(self):
        ...
        code.matrix_cmul(y='velz', x='v', W=numpy.array([0, 0, 1]).reshape(-1, 1))
...
#        
        #Readouts
        code.readout(x=Literal(grid), mesh='velocity', value='velgrid', layout=Literal(layout), resampler='nearest')
        #Take care of rsdfactor for redshift
        code.multiply(x1='velgrid', x2=Literal(self.rsdfactor), y='velgrid')
        code.readout(x=Literal(grid), mesh='premodel', value='predictmass', layout=Literal(layout), resampler='nearest')
        code.assign_component(attribute='displace', value='velgrid', dim=2)
        code.add(x1=Literal(grid), x2='displace', y='rsdpos')
        code.paint(x='rsdpos', mesh='model', layout=Literal(layout), mass='predictmass')
        return code
