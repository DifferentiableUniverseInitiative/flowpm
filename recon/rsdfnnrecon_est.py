...
Om0 = 0.2685 + 0.049
H  = 0.6711 * 100
cosmo = cosmology.clone(Om0 = Om0, H0=H, Ob0 = 0.049)
np.random.seed(100)
...
def recon_model(mesh, datasm, rsdfactor, M0, R0, width, off, istd, x0, nc=FLAGS.nc, bs=FLAGS.box_size, batch_size=FLAGS.batch_size,
...
#
    ...
    masssmf = mtf.cwise(cwise_fingauss, [massf, float_to_mtf(R1, mesh, scalar)] + kv + [tfnc, tfbs], output_dtype=cdtype) 
    masssm = mesh_utils.c2r3d(masssmf, final_field.shape[-3:], dtype=dtype)
    masssm = masssm + 1e-5
    imasssm =  mtf.pow(x, -1.)
    
    vzweights = final_state[1]
    vzweights = mtf.slicewise(lambda x: x[:, :, :, :, -1], 
                        [vzweights],
                        output_dtype=tf.float32,
    ...
    momzsmf = mtf.cwise(cwise_fingauss, [momzf, float_to_mtf(R1, mesh, scalar)] + kv + [tfnc, tfbs], output_dtype=cdtype) 
    momzsm = mesh_utils.c2r3d(momzsmf, momz.shape[-3:], dtype=dtype)

    #Shift
    velzsm = mtf.divide(momzsm, masssm)
    vz = mcomp.cic_readout_fr(velzsm, [X], hr_shape=hr_shape, halo_size=halo_size, splittables=splittables, mesh=mesh)
    vz = mtf.multiply(vz, rsdfactor)
    print("vz : ", vz)
    
    Xrsd = mtf.slicewise(lambda x, vz: x + tf.stack([tf.zeros_like(vz), tf.zeros_like(vz), vz], 4), 
                          [X, vzweights],
                        output_dtype=tf.float32,
    ...
    print(Xrsd)
    modelread = mcomp.cic_readout_fr(model, [X], hr_shape=hr_shape, halo_size=halo_size, splittables=splittables, mesh=mesh)
...
def model_fn(features, labels, mode, params):
    ...
    graph = mtf.Graph()
    mesh = mtf.Mesh(graph, "my_mesh")
    fields, metrics, kv = recon_model(mesh, features['datasm'], features['rsdfactor'], features['M0'], features['R0'], features['w'], \
...
def main(_):
    ...
    hvel = tools.readbigfile('/project/projectdirs/m3058/chmodi/cosmo4d/data/L0400_N0512_S0100_40step/FOF/CMVelocity//')[1:int(bs**3 *numd)]
