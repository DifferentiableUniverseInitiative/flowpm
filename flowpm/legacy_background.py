""" Perturbation module from  https://github.com/rainwoodman/fastpm-python/blob/master/fastpm/background.py"""
...
class MatterDominated(Perturbation):
  ...
  def efunc(self, a):
    return (self.Omega0_m / a**3 + self.Omega0_k / a**2 +
            self.Omega0_lambda)**0.5

  def efunc_prime(self, a):
    return 0.5 / self.efunc(a) * (-3 * self.Omega0_m /
                                  (a**4) + -2 * self.Omega0_k / (a**3))
