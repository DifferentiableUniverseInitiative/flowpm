...
def lpt_prototype(mesh, nc=FLAGS.nc, bs=FLAGS.box_size, batch_size=FLAGS.batch_size,
                  a0=FLAGS.a0, a=FLAGS.af, nsteps=FLAGS.nsteps):
    """
...
def main(_):
    ...
    mesh_devices = ["/job:mesh/task:%d/device:GPU:%d"%(i,j) for i in range(cluster_spec.num_tasks("mesh")) for j in range(FLAGS.gpus_per_task)]
    ...
    initial_conditions, mesh_final_field = lpt_prototype(mesh, bs = FLAGS.box_size,
    ...
    plt.colorbar()
    plt.savefig("figs/pyramid_lpt_%d-b1%d-b2%d.png"%(FLAGS.nc, FLAGS.nx, FLAGS.ny))
    ...
    with tf.Session(server.target) as sess:
        ...
        niter = 10
        start = time.time()
        for i in range(niter): err = sess.run(result)
        end = time.time()
