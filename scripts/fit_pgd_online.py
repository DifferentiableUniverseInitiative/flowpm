...
def main(_):
  ...
  print(stages)
  states = nbody(cosmology, initial_state, stages, FLAGS.nc, FLAGS.B)
  print('Fiducial Simulation done')
  fpath = './fits/L%04d_N%04d_T%02d_B%d/' % (FLAGS.box_size, FLAGS.nc,
                                             FLAGS.nsteps, FLAGS.B)
  ...
  for i in range(len(states)):
    ...
    final_field = flowpm.cic_paint(
        tf.zeros_like(initial_conditions), states[i][1][0])
  ...
  pgdparams = tf.Variable(
      name='pgd',
      shape=(3),
      dtype=tf.float32,
      initial_value=params0,
      trainable=True)
  statespgd, dmstate, params = tfpm.nbody_pgd(
      cosmology,
      initial_state,
