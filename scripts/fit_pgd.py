...
def main(_):
    ...
    Nmodes = initpk(shape=initial_state.shape[1:],boxsize=np.array([FLAGS.box_size, FLAGS.box_size,
                                        FLAGS.box_size]),kmin=np.pi/FLAGS.box_size,dk=2*np.pi/FLAGS.box_size)[1].numpy().real[1:-1]
    ...
    for i in [ia]:
        ...
        k, power_spectrum1 = pkl(final_field1,shape=final_field1.shape,boxsize=np.array([FLAGS.box_size, FLAGS.box_size,
                                             FLAGS.box_size]),kmin=np.pi/FLAGS.box_size,dk=2*np.pi/FLAGS.box_size)
