...
#    
#    yy = mtf.cwise(apply_pwts,
#                        [x, x1, x12],
#                        output_dtype=tf.float32)

##    def padded_splits(x):
##        xhr = mtf.slicewise(lambda x:tf.expand_dims(tf.expand_dims(tf.expand_dims(x, axis=1),axis=1),axis=1),
##                          [x],
##                          output_dtype=tf.float32,
...
##            xhr = mtf.pad(xhr, [halo_size, halo_size], block_size_dim.name)
##
##        for blocks_dim, block_size_dim in zip(hr_shape[1:4], xhr.shape[-3:]):
##            xhr = mpm.halo_reduce(xhr, blocks_dim, block_size_dim, halo_size)
##        return xhr
##
##    
##    xhr = padded_splits(x)
##    x1hr = padded_splits(x1)
##    x12hr = padded_splits(x12)
##
##    y = mtf.reshape(xhr, xhr.shape+[mtf.Dimension('c_dim', 1)])
##    y1 = mtf.reshape(x1hr, xhr.shape+[mtf.Dimension('c_dim', 1)])
##    y12 = mtf.reshape(x12hr, xhr.shape+[mtf.Dimension('c_dim', 1)])
##
##
##    kernel = tf.constant(dtype=dtype, value=kernel)
##    fd_dim = mtf.Dimension("fd", size)
##    fh_dim = mtf.Dimension("fh", size)
##    fw_dim = mtf.Dimension("fw", size)
##    #cdim27 = mtf.Dimension("c27", 27)
##    
##    kernel =  mtf.import_tf_tensor(mesh, kernel, shape=[fd_dim, fh_dim, fw_dim, y.shape[-1], mtf.Dimension("c27", 27)])
##    pwts, pbias, pmx, psx = ppars
##
##    print('Kernel : ', kernel)
##    print('y : ', y)
##    p = mtf.Conv3dOperation(y, kernel, strides=(1,1,1,1,1), padding='SAME').outputs[0]
##    print('p : ', p)
##    
##    
##    p1 = mtf.Conv3dOperation(y1, kernel, strides=(1,1,1,1,1), padding='SAME').outputs[0]
##    print('p1 : ', p1)
##    p12 = mtf.Conv3dOperation(y12, kernel, strides=(1,1,1,1,1), padding='SAME').outputs[0]
##    print('p12 : ', p12)
##
##
##    print(p.shape)
##    yy = mtf.concat([p, p1, p12], "c27")
##    
##    print('yy : ', yy)
##    yy = yy - pmx
##    yy = yy / psx
