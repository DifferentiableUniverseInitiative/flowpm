...
def gradients_prototype(mesh, tfic, nc=FLAGS.nc, bs=FLAGS.box_size, batch_size=FLAGS.batch_size,
    ...
    #mtfic = tf.placeholder(dtype=dtype, shape=[batch_size, nc, nc, nc])
    initc = mtf.import_tf_tensor(mesh, tfic, shape=shape)
...
def main(_):
    ...
    # Run normal flowpm
    tfic = tf.placeholder(dtype=ic.dtype, shape=ic.shape)
    ...
    tfsum = tf.reduce_sum(tfinal_field**2)
    tfgrads = tf.gradients(tfsum, tfic)

    with tf.Session(server.target) as sess:
        a,b, gb = sess.run([tfinal_field, tfsum, tfgrads], feed_dict={tfic:ic})
        gb = gb[0]

        for n in range(10):
            print(n)
            i, j, k = np.random.randint(0, FLAGS.nc, 3)
            #i, j, k = 21, 34, 52
            ic0, ic1 = ic.copy(), ic.copy()
            ic1[0, i, j, k] *= 1.05
            ic0[0, i, j, k] *= 0.95
            b1 = sess.run(tfsum, feed_dict={tfic:ic1})
            b0 = sess.run(tfsum, feed_dict={tfic:ic0})
            
            #bgrad = (b0-b1)[0, i, j, k]/(ic0-ic1)[0, i, j, k]
            #cgrad = (c0-c1)[0, i, j, k]/(ic0-ic1)[0, i, j, k]
            #bgrad = (b0.sum() - b1.sum())/(ic0-ic1)[0, i, j, k]
            #cgrad = (c0.sum() - c1.sum())/(ic0-ic1)[0, i, j, k]
            bgrad = (b0 - b1)/((ic0-ic1)[0, i, j, k])
            
            print(b0, b1)
            print('Single : ', i, j, k, gb[0, i, j, k], bgrad, gb[0, i, j, k]/bgrad)
        
    #    
    # Create computational graphs
    
    initial_conditions, mesh_final_field, stages = gradients_prototype(mesh, tfic, nc=FLAGS.nc,
                                                                       batch_size=FLAGS.batch_size, dtype=dtype)

    print(initial_conditions)

    #mesh_sum  = mtf.square(mesh_final_field)
    #mesh_sum = mtf.reduce_sum(meshsq)
    #mesh_sum = mtf.reduce_sum()
    #mesh_sum = mtf.reduce_sum(mtf.multiply(initial_conditions, initial_conditions))
    mesh_sum = mtf.reduce_sum(mtf.multiply(mesh_final_field, mesh_final_field))

    #mtfgrads = mtf.gradients([mtf.cast(mesh_sum, tf.float32)], [mtf.cast(initial_conditions, tf.float32)])
    #mtfgrads = mtf.gradients([mesh_final_field], [initial_conditions])
    mtfgrads = mtf.gradients([mesh_sum], [initial_conditions])
    #mtfgrads = mtf.gradients([mesh_sum], [mesh_final_field])
    print(mtfgrads)
    #mesh_sum, mtfgrads = mesh_final_field, [mesh_final_field]
    ...
    meshsum = lowering.export_to_tf_tensor(mesh_sum)
    initc  = lowering.export_to_tf_tensor(initial_conditions)
    grads  = lowering.export_to_tf_tensor(mtfgrads[0])


    with tf.Session(server.target) as sess:
        a,b,c = sess.run([initc, tfinal_field, result], feed_dict={tfic:ic})
        gb,gc = sess.run([tfgrads, grads], feed_dict={tfic:ic})
        gb = gb[0]
        print(gb/gc)
        ...
        for n in range(10):
            ...
            ic1[0, i, j, k] *= 1.05
            ic0[0, i, j, k] *= 0.95
            i1, b1,c1 = sess.run([initc, tfsum, meshsum], feed_dict={tfic:ic1})
            i0, b0,c0 = sess.run([initc, tfsum, meshsum], feed_dict={tfic:ic0})
            ...
            #cgrad = (c0.sum() - c1.sum())/(ic0-ic1)[0, i, j, k]
            bgrad = (b0 - b1)/((ic0-ic1)[0, i, j, k])
            cgrad = (c0 - c1)/((ic0-ic1)[0, i, j, k])
            igrad = (i0-i1)[0, i, j, k]/(ic0-ic1)[0, i, j, k]
            
            print(b0, b1, c0, c1)
            print('Init : ', i, j, k,  igrad)
            print('Single : ', i, j, k, gb[0, i, j, k], bgrad, gb[0, i, j, k]/bgrad)
            print('Pyramid : ', i, j, k, gc[0, i, j, k], cgrad, gc[0, i, j, k]/cgrad)
    #    
    np.save('init', a)
    ...
    plt.savefig("numcomparison.png")

    
    plt.figure(figsize=(8,3))
    plt.subplot(131)
    plt.imshow(gb[0].sum(axis=2))
    plt.colorbar()
    plt.title('single GPU')

    plt.subplot(132)
    plt.imshow(gc[0].sum(axis=2))
    plt.colorbar()
    plt.title('Mesh')

    plt.subplot(133)
    plt.imshow((gb-gc)[0].sum(axis=2))
    plt.colorbar()
    plt.title('Residual')

    plt.savefig("numgradientscomparison.png")
