...
def nbody_prototype(mesh, infield=False, nc=FLAGS.nc, bs=FLAGS.box_size, batch_size=FLAGS.batch_size,
    ...
    field = mcomp.fr_to_hr(initc, hr_shape, halo_size, splittables, mesh)

    
    high = field
    low = mcomp.downsample_hr_to_lr(field, lr_shape, hr_shape, downsampling_factor, halo_size, splittables, mesh)
...
def main(_):
    ...
    devices = ["/job:mesh/replica:0/task:%d/device:GPU:%d"%(i,j) for i in range(cluster_spec.num_tasks("mesh")) for j in range(FLAGS.gpus_per_task)]
    devices = [""] * mesh_size
